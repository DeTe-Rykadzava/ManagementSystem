...
public class OrderPaymentTypesViewModel : RoutableViewModelBase
{
    public override string ViewModelViewPath { get; } = "payment-types";
    ...
    // fields
    public ObservableCollection<OrderPaymentTypeViewModel> Types { get; } = new ();
    ...
    public ReactiveCommand<OrderPaymentTypeViewModel, Unit> DeleteTypeCommand { get; }

    public OrderPaymentTypesViewModel(IOrderPaymentTypeService orderPaymentTypeService, IDialogService dialogService)
    {
        ...
        AddTypeCommand = ReactiveCommand.CreateFromTask(async () =>
            {
                var result = await _orderPaymentTypeService.AddTypeAsync(NewTypeName);
            ...
            canAddNewType);
        DeleteTypeCommand = ReactiveCommand.CreateFromTask(async (OrderPaymentTypeViewModel type) =>
        {
            var dialogResult =
            ...
            if(dialogResult == ButtonResult.No)
                return;
            var deleteResult = await _orderPaymentTypeService.RemoveTypeAsync(type.Id);
    ...
    public override async Task OnShowed()
    {
        Task.Run(new Action(async () =>
        {
            ...
            var categoriesResult = await _orderPaymentTypeService.GetAllAsync();
